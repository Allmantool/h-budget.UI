...
Required discovery: Read AGENTS.md and the Angular/Nx skill; for non-trivial work create/update the task specification; inspect the implementation, existing tests, provider/state/mapping contracts, actual test target, and git status.
Expected validation: Add deterministic behavior-focused coverage; run relevant tests and lint/build where risk warrants.
Self-review: Inspect the complete diff, confirm RED evidence where practical and that tests protect the stated behavior without weakening existing coverage, fix findings, rerun tests.
